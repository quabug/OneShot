...
public sealed class OneShotGenerator : IIncrementalGenerator
{
    ...
    {
        // Pipeline A: Types with [Injectable] class-level attribute
        var injectableTypes = context.SyntaxProvider.ForAttributeWithMetadataName(
            "OneShot.InjectableAttribute",
            predicate: static (node, _) => node is TypeDeclarationSyntax,
            transform: static (ctx, ct) => TypeModelExtractor.Extract(
                (INamedTypeSymbol)ctx.TargetSymbol, ctx.SemanticModel.Compilation)
        ...
        var combined = injectableTypes.Collect()
